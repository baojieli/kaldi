...
for id in $ids; do
  ...
  if [ "$(wc -l temp7.txt | awk '{print $1}')" != "$(wc -l $script | awk '{print $1}')" ]; then
    echo "for id: $id"
    echo "ctm text: $(wc -l temp1.txt)"
    echo "ctm times: $(wc -l temp6.txt)"
    echo "script: $(wc -l $script)"
    echo "new segs: $(wc -l temp7.txt)"
#    cp temp7.txt ${id}_err_segments.txt
#    cp $script ${id}_err_script.txt
  fi
