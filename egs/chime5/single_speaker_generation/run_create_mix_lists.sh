...
if [ $stage -le 1 ]; then
  for dset in dev train; do
    mkdir -p tmp_mix/U01/$dset
    awk -v mic=U01 -v dset=$dset '{print mic"/"dset"/"$1".wav"}' \
      data/${dset}_final/U01/segments > tmp_mix/U01/$dset/filelist.txt
  done

  mkdir -p mixes
  local/filelist_to_mixlist.py --target-mix-num 5000 tmp_mix/U01/dev/filelist.txt \
    mixes/mix_2_spk_U01_dev.txt
  local/filelist_to_mixlist.py --target-mix-num 20000 tmp_mix/U01/train/filelist.txt \
...
if [ $stage -le 2 ]; then
  ...
  for dset in dev train; do
    ...
    sed -e "s/U01\/${dset}\///g;s/\.wav//g" mixes/mix_2_spk_U01_${dset}.txt |\
      utils/apply_map.pl -f 1 U01_to_close_talking_map.txt |\
      utils/apply_map.pl -f 3 U01_to_close_talking_map.txt |\
      awk -v dset=$dset '{print "close_talking/"dset"/"$1".wav "$2" close_talking/"dset"/"$3".wav "$4}' \
