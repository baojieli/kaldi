// featbin/concat-vad-spk-segments.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Concatenate vad files of segments for a speaker,\n"
        "so the output file has the sum of the num-frames of the inputs.\n"
        "Usage: concat-vad-spk-segments <in-rspecifier> <spk2utt-rxfilename> <out-wspecifier>\n"
        " e.g. concat-vad-spk-segments scp:vad.scp spk2utt ark,scp:full_vad.ark,full_vad.scp\n"
    ...
    while (std::getline(spk2utt.Stream(),line)) {
      ...
      std::vector<Vector<BaseFloat> > feats(split_line.size()-1);
